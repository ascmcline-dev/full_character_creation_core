...
from .nodes_v258 import _piercing_prompt_v258, _scar_record_prompt_v258, _tattoo_record_prompt_v258
...
def _region_marks(profile: dict[str, Any], regions: set[str], presentation: str) -> str:
    ...
    for record in profile.get("tattoo_records", []) if isinstance(profile.get("tattoo_records"), list) else []:
        if isinstance(record, dict) and _record_intersects(record, regions):
            text = _tattoo_record_prompt_v258(record, None)
    ...
    if piercing_records:
        parts.append(_sentences(*(_piercing_prompt_v258(record, {}) for record in piercing_records)))
    for record in profile.get("scar_mole_beauty_mark_records", []) if isinstance(profile.get("scar_mole_beauty_mark_records"), list) else []:
        if isinstance(record, dict) and _record_intersects(record, regions):
            parts.append(_scar_record_prompt_v258(record))
...
class FCCKreaBlueprintDatasetDirector:
    ...
    FUNCTION = "direct"
    DESCRIPTION = (
        "Stage 2 Krea2 director aligned to the canonical V2.4.18 anatomy object. Plan names advertise base counts, nonbinary identity no longer blends body anatomy, and the opt-in extreme plan contains true single-detail close-ups rather than regional torso crops."
    )
    ...
    def direct(self, character_blueprint, dataset_plan, project_name, starting_seed, variations_per_shot, prompt_suffix=""):
        ...
        plan_json = json.dumps({
            "schema": "FCC_KREA_STAGE2_CANONICAL_ATLAS_V258", "schema_version": 8,
        ...
        dashboard = "\n".join([
            "FCC STAGE 2 — KREA CANONICAL DOCUMENTATION V2.4.18",
...
class FCCFaceAngleDatasetDirector(FCCFaceAngleDatasetDirectorV256):
    DESCRIPTION = (
        "Stage 3 Qwen director registered to V2.4.18. Qwen remains manually reviewed and is intentionally unchanged while Stage 0 and Stage 2 are validated."
    )
